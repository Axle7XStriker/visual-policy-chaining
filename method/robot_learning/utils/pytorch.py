...
def to_tensor(x, device):
    if isinstance(x, dict):
        return OrderedDict(
            [(k, torch.as_tensor(v if isinstance(v, torch.Tensor) else v.copy(), device=device).float()) for k, v in x.items()]
        )
...
def center_crop(img, out=84):
    ...
    return img.mean(axis=1, keepdim=True) if isinstance(img, torch.Tensor) else img.mean(axis=1, keepdims=True)
...
def center_crop_images(image, out=84):
    ...
    image = image.float() if isinstance(image, torch.Tensor) else image.astype('float64')
    return image.mean(axis=1, keepdim=True) if isinstance(image, torch.Tensor) else image.mean(axis=1, keepdims=True)
